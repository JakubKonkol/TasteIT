...
public class BadgeTrackingAspect {
    ...
    private final Map<String, Consumer<String>> badgeLogicMap = new HashMap<>();

    public BadgeTrackingAspect() {
        badgeLogicMap.put("badge_001", this::firstRecipe);
//        badgeLogicMap.put("badge_002", this::risingChefFood);
//        badgeLogicMap.put("badge_003", this::risingChefFoodDrink);
        badgeLogicMap.put("badge_004", this::masterChef);
...
//        badgeLogicMap.put("badge_012", this::recipeConnoisseur);
    ...
    @Before("@annotation(trackBadgesProgress) && args(.., sessionToken)")
    public void handleBadgeTracking(JoinPoint joinPoint, TrackBadgesProgress trackBadgesProgress, String sessionToken) {
        MethodSignature signature = (MethodSignature) joinPoint.getSignature();
        Method method = signature.getMethod();
        TrackBadgesProgress[] trackBadgesProgresses = method.getAnnotationsByType(TrackBadgesProgress.class);


        for (TrackBadgesProgress trackBadge : trackBadgesProgresses) {
            String badgeId = trackBadge.badgeId();
            if (badgeLogicMap.containsKey(badgeId)) {
                badgeLogicMap.get(badgeId).accept(sessionToken); // Wywołanie logiki dla odznaki
            }
        }
    ...
    private void firstRecipe(String sessionToken) {
        ...
        if (userService.getCurrentUserBySessionToken(sessionToken).getBadges().stream().noneMatch(badge -> badge.getBadgeId().equals("badge_001")))
            badgeService.grantBadgeToUser("badge_001", userId, sessionToken);
    }
    private void masterChef(String sessionToken) {
        String userId = userService.getCurrentUserBySessionToken(sessionToken).getUserId();
        if (userService.getCurrentUserBySessionToken(sessionToken).getBadges().stream().noneMatch(badge -> badge.getBadgeId().equals("badge_004")))
            ...
            badgeService.grantBadgeToUser("badge_004", userId, sessionToken);
        else
            badgeService.updateBadgeProgress(sessionToken,"badge_004");
